...
int main(void)
{
  ...
  __HAL_RCC_GPIOO_CLK_ENABLE(); // oder welcher Port deine LED ist
  GPIO_InitTypeDef gpio = {0};
  gpio.Pin = GPIO_PIN_1;
  gpio.Mode = GPIO_MODE_OUTPUT_PP;
  gpio.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOO, &gpio);
  // Checkpoint FSBL is alive
  for (int i=0;i<2;i++) {
  ...
  RISAF_BaseRegionConfig_t RISAF_PSRAM_Config = {0};

  RISAF_PSRAM_Config.Filtering = RISAF_FILTER_ENABLE;
  RISAF_PSRAM_Config.Secure = RIF_ATTRIBUTE_NSEC;
  RISAF_PSRAM_Config.PrivWhitelist = RIF_CID_1 | RIF_CID_2;
  RISAF_PSRAM_Config.ReadWhitelist = RIF_CID_1 | RIF_CID_2;
  RISAF_PSRAM_Config.WriteWhitelist = RIF_CID_1 | RIF_CID_2;
  HAL_RIF_RISAF_ConfigBaseRegion(RISAF1, 0, &RISAF_PSRAM_Config);
