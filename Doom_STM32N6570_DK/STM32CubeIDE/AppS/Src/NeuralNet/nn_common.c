...
static boolean NN_RayCallback(intercept_t *in)
{
    ...
    if (li->backsector)
    {
        ...
        if (open_top - open_bottom >= 56 * FRACUNIT) // 56 = Spielerhöhe
            return true;
    }
...
float NN_CastRay(fixed_t x, fixed_t y, angle_t angle, fixed_t max_dist)
{
    ...
    ray_hit_dist = FRACUNIT; // max. Abstand (100%) entspricht 65536
    P_PathTraverse(x, y, x + dx, y + dy, PT_ADDLINES, NN_RayCallback);

    float dist_frac = (float)ray_hit_dist / (float)FRACUNIT; // zw. 0.0 und 1.0f
    float max_f     = (float)max_dist / (float)FRACUNIT; // max_dist = 2048*FRACUNIT, max_f = 2048
    return dist_frac * max_f; // 0.x * 2048
}
